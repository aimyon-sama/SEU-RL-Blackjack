...
def main():
    ...
    parser.add_argument("--model", type=Path, default=Path(__file__).with_name("model") / "player1_q_model.pkl")
    ...
    for _ in range(args.games):
        ...
        if args.verbose:
            ...
            with contextlib.redirect_stdout(io.StringIO()):
                rank = game.run()[0]
        rank_sum += int(rank)
        final_chips_sum += int(getattr(player, "chips", 0))

    rounds = max(player.rounds_finished, 1)
    print(f"games={args.games}")
    print(f"avg_rank={rank_sum / args.games:.3f}")
    print(f"avg_final_chips={final_chips_sum / args.games:.3f}")
